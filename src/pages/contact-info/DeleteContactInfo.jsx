...
const DeleteContactInfo = () => {
  ...
  const deleteMasterAlumni = async () => {
    ...
    try {
      setLoading(true);
      await axios
        .delete(`${envConfig.contactFormDataUrl}/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        .then((res) => {
          const resStatus = localStorage.getItem(`res-send-${id}`) || null;
          if (resStatus) {
            localStorage.removeItem(`res-send-${id}`);
          }
          setAlertMessage({
